...
shiftCipher str n = foldr (+++) "" (map (\x = fromNum (toNum x) n) [ a \\ a <-: str ])

toNum :: Char -> Int
toNum c = ((toInt c) - 65)

fromNum :: Int Int -> String
fromNum z n = toString (toChar ((z + n) rem 26 + 65))
...
check c arr = and (map (\x = (dist x c) <= 2.0 ) [ a \\ a <-: arr ])
...
instance + Library
where
	(+) l1 l2 = { lib_name = l1.lib_name +++ l2.lib_name, books = { a \\ a <- removeDup ([ b \\ b <-: l1.books] ++ [b \\ b <-: l2.books]) } }
...
instance + {String}
where
	(+) arr1 arr2 = { foldr (\x y = toString x +++ toString y) ("") (flatten [ [c] ++ [d] \\ c <-: a & d <-: b ]) +++ (rest a b) \\ a <-: arr1 & b <-: arr2 }
	where
		rest :: String String -> String
		rest s1 s2
		| size s1 == size s2 = ""
		| size s1 > size s2 = foldr (\x y = toString x +++ toString y) ("") [ s1.[a] \\ a <- [(size s2)..((size s1) - 1)] ]
		= rest s2 s1
...
AVL_prop_check (Node _ le ri)
| abs ((AVL_prop_check_aux le) - (AVL_prop_check_aux ri)) <= 1 = and ([AVL_prop_check le] ++ [AVL_prop_check ri])
= False

AVL_prop_check_aux :: (Tree Int) -> Int
AVL_prop_check_aux Leaf = 0
AVL_prop_check_aux (Node _ le ri) = 1 + max (AVL_prop_check_aux le) (AVL_prop_check_aux ri)
...
transform tree = transformAux tree (toList tree)

toList :: (Tree Int) -> [Int]
toList Leaf = []
toList (Node x le ri) = ((toList le) ++ [x] ++ (toList ri))

transformAux :: (Tree Int) [Int] -> (Tree Int)
transformAux Leaf _ = Leaf
transformAux (Node x le ri) list = Node (sum (filter ((<=)x) list)) (transformAux le list) (transformAux ri list)
...
alterArray arr = { (a*(fst3 m) , b*(snd3 m) , c*(thd3 m)) \\ (a,b,c) <-: arr }
where
	m = minim arr
	
minim :: {(Int, Int, Int)} -> (Int,Int,Int)
minim arr = foldr (\x y | abs (fst3 x + snd3 x + thd3 x) < abs (fst3 y + snd3 y + thd3 y) = x = y) (last list) (init list)
where
	list = [ (a,b,c) \\ (a,b,c) <-: arr | a <> b && b <> c && c <> a]
...
getOldestMarriage arr = toFamily (foldr (\x y | (x.yearOfMarriage) <= (y.yearOfMarriage) = x = y) (last list) (init list)).fullName
where
	list = [ fst a \\ a <-: arr | toFamily ((fst a).fullName) == toFamily ((snd a).fullName) ] 

toFamily :: String -> String
toFamily str = foldr (\x y = toString x +++ toString y) ("") (tl (dropWhile ((<>)' ') [ a \\ a <-: str ]))
...
neighbours list = [ ColorList!!(((ColorID a) + 2) rem 7) \\ a <- list ]
...
getBTDiameter tree = foldr (\x y = max x y) (last list) (init list)
where
	list = getBTDiameterAux tree

getBTDiameterAux :: BinaryTree2 -> [Int]
getBTDiameterAux BTLeaf2 = [1]
getBTDiameterAux (BTNode2 le ri) = ([(CheckDepth le) + (CheckDepth ri) + 1] ++ (getBTDiameterAux le) ++ (getBTDiameterAux ri))

CheckDepth :: BinaryTree2 -> Int
CheckDepth BTLeaf2 = 1
CheckDepth (BTNode2 le ri) = 1 + (max (CheckDepth le) (CheckDepth ri))
