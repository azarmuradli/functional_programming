...
getBestBScStudent list = maxAvg [ (a.id,a.grades) \\ a <- list | isBSc a]
where
	isBSc :: Student -> Bool
	isBSc {id=_, level=BSc, grades=_} = True
	isBSc s = False
	
maxAvg :: [(String,[Int])] -> String
maxAvg [x] = fst x
maxAvg [x,y:xs]
| f >= s = maxAvg [x:xs]
= maxAvg [y:xs]
...
count str = { a \\ a <- (toBag stringToList) | snd a > 1 }
where
	stringToList = [ a \\ a <-: str ]
	toBag list = map (\x = ( x , length (filter (\y = y == x) list) )) (removeDup list)
...
instance ~ [a] | Ord,toInt a
where
	~l1 = [ a \\ a <- l1 | (toInt a) > 0 ]

instance - [a] | Eq a
where
	(-) l1 l2 = [ a \\ a <- l1 | not (isMember a l2) ]
...
Insert (Pointer x next) str = Pointer x (Insert next str)
...
Reverse x = ReverseAux x (reverse (collect x))

collect :: (LinkedList String) -> [String]
collect Nil = []
collect (Pointer x next) = [x] ++ (collect next)

ReverseAux :: (LinkedList String) [String] -> (LinkedList String)
ReverseAux Nil list = Nil
ReverseAux (Pointer _ next) [x:xs] = Pointer (x) (ReverseAux next xs)
...
delete Nil _ = Nil
delete (Pointer x next) str
| x == str = next//delete next str
= Pointer x (delete next str)
...
concat Nil Nil = Nil
concat Nil (Pointer x next) = Pointer x (concat Nil next)
concat (Pointer x next) s = Pointer x (concat next s)
...
findFakes arr = { x \\ x <- (findFakesAux [ a \\ a <-: arr ] [ a \\ a <-: arr ] []) }

findFakesAux :: [BasicPersonAccount] [BasicPersonAccount] [BasicPersonAccount] -> [BasicPersonAccount]
findFakesAux [] _ _ = []
findFakesAux [x:xs] list mat
| matches && (not (isPresent x mat)) = [{ name = x.name, age = x.age, inQueue = False, ip = x.ip }] ++ (findFakesAux xs list (mat ++ [x]))
| (not (isPresent x mat)) = [{ name = x.name, age = x.age, inQueue = x.inQueue, ip = x.ip }] ++ (findFakesAux xs list (mat ++ [x]))
= findFakesAux xs list (mat ++ [x])
where
	matches = or (map (\a | (a.name == x.name && a.ip == x.ip && a.age <> x.age) = True = False ) list)

isPresent :: BasicPersonAccount [BasicPersonAccount] -> Bool
isPresent _ [] = False
isPresent z [x:xs]
| z.name == x.name && z.ip == x.ip = True
= isPresent z xs
...
largestMat :: {[[Int]]} -> [[Int]]
largestMat arr = arr.[(largestMatInd diagSum diagSum 0)]
where
	diagSum = [ sum [ b!!n \\ b <- (a) & n <- [0..((length a) - 1)] ] \\ a <-: arr ]

largestMatInd :: [Int] [Int] Int  -> Int
largestMatInd [x:xs] list n
| or (map (\z | x >= z = False = True) list) = largestMatInd xs list (n+1)
= n
